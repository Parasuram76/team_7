...
// @route   GET /api/tools/socratic-sessions
// @desc    Get all Socratic chat sessions for the user
// @access  Private
router.get('/socratic-sessions', async (req, res) => {
  try {
    const userId = req.user._id;
    // Find sessions starting with 'socratic-'
    const sessions = await ChatHistory.find({
      userId,
      sessionId: { $regex: /^socratic-/ }
    }).sort({ updatedAt: -1 }).select('sessionId messages updatedAt');

    const summaries = sessions.map(session => {
      const firstUserMsg = session.messages.find(m => m.role === 'user');
      let preview = firstUserMsg ? firstUserMsg.parts[0].text.substring(0, 50) : "New Conversation";
    ...
    console.error("Error fetching Socratic sessions:", error);
    res.status(500).json({ message: "Failed to fetch sessions" });
  }
});

// @route   GET /api/tools/socratic-session/:sessionId
// @desc    Get a specific Socratic session
// @access  Private
router.get('/socratic-session/:sessionId', async (req, res) => {
  try {
...
// @route   DELETE /api/tools/socratic-session/:sessionId
// @desc    Delete a specific Socratic session
// @access  Private
router.delete('/socratic-session/:sessionId', async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user._id;
...
// @route   POST /api/tools/socratic-chat
// @desc    Interact with the Socratic Tutor
// @access  Private
router.post('/socratic-chat', async (req, res) => {
  ...
  try {
    ...
    auditLog(req, 'TOOL_USAGE_SOCRATIC_TUTOR', {
      messageLength: message.length
    });

    const pythonResponse = await axios.post(`${pythonServiceUrl}/socratic_chat`, {
      message,
      history,
      apiKey
    }, { timeout: 60000 });
    ...
    // Return the sessionId so frontend knows where we are
    res.status(200).json({ ...pythonResponse.data, sessionId });

  } catch (error) {
    auditLog(req, 'TOOL_USAGE_SOCRATIC_TUTOR_FAILURE', {
      error: error.message
    });
    const errorMsg = error.response?.data?.error || error.message;
    console.error(`[Node Socratic] Error: ${errorMsg}`);
    res.status(error.response?.status || 500).json({ message: errorMsg });
  }
});
